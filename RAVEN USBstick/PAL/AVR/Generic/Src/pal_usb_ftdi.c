 ...
 *
 * $Id: pal_usb_ftdi.c 25875 2011-03-07 13:02:08Z felix.varghese $
 *
...
#define USB_SELECT()        do{ LED_ADDR_DEC_PORT &= ~_BV(6);   \
                                LED_ADDR_DEC_DDR |= _BV(6);     \
                                LED_ADDR_DEC_PORT &= ~_BV(7);   \
                                LED_ADDR_DEC_DDR |= _BV(7); }while(0)

#define USB_DESELECT()      do{ LED_ADDR_DEC_PORT |= _BV(6);    \
                                LED_ADDR_DEC_DDR |= _BV(6);     \
                                LED_ADDR_DEC_PORT |= _BV(7);    \
                                LED_ADDR_DEC_DDR |= _BV(7); }while(0)
#endif  /* USB_FTDI_BITBANG */
/* === Globals ============================================================= */
/**
 * USB transmit buffer
 */
...
static void usb_putc(void)
{
    if (usb_tx_count > 0)
    {
        ...
        /* Pull WR# low so that the buffer reverses direction */
        USB_CTRL_WR_LO();
        ...
        /* Select USB again (this will also cause USB_WR to get asserted) */
        USB_SELECT();

        /* Set port back to input */
        USB_DATA_SETINP();

        /* Pull WR# back to high */
        USB_CTRL_WR_HI();
#else
